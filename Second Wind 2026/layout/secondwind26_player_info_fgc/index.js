...
hidingAnimation
  ...
  // Info grid exits first (last in → first out)
  .to([".info_grid"], {
    duration: 0.6,
    y: "300px",
    autoAlpha: 0,
    ...
    if (!isTeams) {
      ...
      for (const [t, team] of teams.entries()) {
        ...
        for (const [p, player] of players.entries()) {
          ...
          // if (player.seed) infoGridHtml += `<div class="seed">Seed ${player.seed}</div>`;
          ...
          // if((player.country.code == "NZ" || player.country.code == "AU") && player.state.asset) {
          //   SetInnerHtml($(`.p${t + 1} .flagstate`), `<div class="flagstate"><img class='flag' src='../../${player.state.asset}' /></div>`);
          // } else if (player.country.asset) {
          //   SetInnerHtml($(`.p${t + 1} .flagstate`), `<div class="flagstate flagcountry"><img class='flag' src='../../${player.country.asset}' /></div>`);
          // }
